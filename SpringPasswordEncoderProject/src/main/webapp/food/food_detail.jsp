...
<script type="text/javascript"
	src="https://dapi.kakao.com/v2/maps/sdk.js?appkey=47419c1fd6fbc1a48dd8ee5c0d04c75b&libraries=services"></script>
<link rel="stylesheet"
...
<body>
	<div class="container">
		<div class="row">
			...
			<table class="table">
				<tr>
					<td class="text-center" v-for="img in poster"><img :src="img"
						style="width: 100%"></td>
				</tr>
			</table>
		</div>
			...
			<div class="col-sm-7">
				...
				<table class="table">
					<tr>
						<td colspan="2">
							<h4>
								<span id="name">{{food_detail.name}}</span>&nbsp;<span
									style="color: orange;">{{food_detail.score}}</span>
							</h4>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td>
							<table class="table" v-for="re in reply_data">
								<tr>
									<td class="text-left"><span style="color: blue">{{re.name}}</span>&nbsp;{{re.dbday}}
									</td>
									<td class="text-right">
										<span v-if="re.id==food_detail.sessionId"> 
											<input type="button" class="btn btn-xs btn-success" value="수정" :id="'up'+re.no"
													@click="replyUpdateForm(re.no)">
											<input type="button" class="btn btn-xs btn-danger" value="삭제"
													@click="replyDelete(re.no)">
										</span>
									</td>
								</tr>
								<tr>
									<td colspan="2" class="text-left" valign="top"><pre
											style="white-space: pre-wrap; background-color: white; border: none;">{{re.msg}}</pre>
									</td>
								</tr>
								<tr style="display: none" :id="'reply'+re.no" class="updates">
									...
									<input type="button" value="댓글수정"
										style="width: 80px; height: 94px; background-color: green; color: white;"
										@click="replyUpdate(re.no)"></td>
								</tr>
							</table>
						</td>
					</tr>
				...
				<table class="table" v-if="food_detail.sessionId!=null">
					<tr>
						<td>
							<!-- ref는 텍스트에어리어 제어용(포커스 주기) / 모델은 data에 값 주입용 --> 
	...
	<script>
		new Vue({
			el:'.container',
			data:{
				fno:${fno},			// EL표현식 => Controller에서 넘어온 fno 값
				food_detail:{},
				reply_data:[],
				msg:'',				// v-model과 연결
				menu:[],
			...
			mounted:function(){
				...
				// 3. Jquery => window.onload (화면이 출력되자마자)
			...
			methods:{
				...
				addScript:function(){
					...
					script.src='https://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=47419c1fd6fbc1a48dd8ee5c0d04c75b&libraries=services'
				...
				replyUpdate:function(no){
					...
					axios.post('../food/reply_update_vue.do',null,{	// updated => 변경된 데이터값
						params:{									// HTML이 변경된 상태로 출력되기 위해서
							no:no,									// data에 설정한 변수가 변경되어야 한다
							msg:msg,
							fno:this.fno
						}
					}).then(response=>{
