  ...
  <main class="container clear"> 
    ...
    <div id="dialog" title="맛집 상세보기" v-if="isShow">
    	...
    	<table class="table">
    		<tr>
    			<td colspan="2">
    				<h2>{{food_detail.name}}&nbsp;<span style="color: orange;">{{food_detail.score}}</span></h2>
    			</td>
    		</tr>
    		<tr>
    			<td width=25%>주소</td>
    		...
    		<tr v-if="food_detail.menu!='no'">
    			...
    			<td width=75%>
    				<ul>
    					<li v-for="menu in menus" v-if="menu!=''">{{menu}}원</li>
    				</ul>
    			</td>
    		</tr>
    	</table>
...
<script>
	new Vue({
		...
		data:{
			...
			food_detail:{},
			page_list:{},
			/* page_list로 페이징을 하지만, 작업시 불편하므로 변수를 아래에 설정
										why? page_list.curpage 로 설정해야하므로		*/
			curpage:1,
			totalpage:0,
			startPage:0,
			endPage:0,
		...
		methods:{
			...
			foodDetail:function(fno,bool){
				...
				axios.get('http://localhost/web/food/food_detail_vue.do',{
					...
					this.posters=this.food_detail.poster.split("^")	// split은 배열로 문자열 저장
