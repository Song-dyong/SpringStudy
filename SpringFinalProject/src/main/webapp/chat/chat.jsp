...
/* on => CallBack함수 (자동호출) */
function onOpen(event){	/* 연결 되었을 때, 처리 */
	alert("채팅서버와 연결되었습니다.")
}
function onClose(event){	/* 퇴장시 처리 */
	alert("채팅서버와 연결이 종료되었습니다.")
}
function onMessage(event){	/* 채팅 메세지 전송시 처리 */
	// msg: 데이터 전송 , msg:은 구분자 ex) roomin:  
	let data=event.data;
	...
	<div class="wrapper row3">
		...
		<div class="row myrow">
			<table class="table">
				<tr>
					<td class="inline">
						이름:<input type="text" id=name size=15 class="input-sm" readnoly value="${sessionScope.name }">
				...
				<tr>
					<td class="inline">
						...
						<input type="button" value="전송" class="btn btn-sm btn-info">
					</td>
				</tr>
			</table>
		</div>
