...
<body>
	<div class="container">
		<div class="row">
			<table class="table">
				...
				<tr>
					<td width=65%>
						<span style="color:green; font-size:8px">첫구매 할인</span>
						&nbsp;<b>{{goods_detail.fp}}</b>
					</td>
				</tr>
				<tr>
				<%-- 
					this.$refs.account => ref에 저장된 값을 가져올 수 있음
					$('#account') Jquery의 EL과 같음
				 --%>
					<td width=65%>
						수량:<select name="account" ref="account" @change="change()">
							<option>1</option>
							<option>2</option>
							<option>3</option>
							<option>4</option>
				...
				<tr>
					<td width=65%>
						<input type=submit value="Cart" class="btn btn-sm btn-success" ref="cartBtn">
					</td>
				</tr>
			</table>
		</div>
	...
	<script>
		new Vue({
			el:'.container',
			data:{
				/* ${no}는 Java에서 보낸 값 (detail.do 에서 모델에 no 저장해서 넘어옴) */
				no:${no},
				goods_detail:{},
				total:0
			},
