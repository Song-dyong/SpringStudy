...
public interface SeoulLocationDAO extends JpaRepository<SeoulEntity, Integer> {
	...
	@Query(value="SELECT * FROM seoul_nature ORDER BY no ASC LIMIT :start, 20",nativeQuery=true)
	...
	@Query(value="SELECT * FROM seoul_shop ORDER BY no ASC LIMIT :start, 20",nativeQuery=true)
